...
int bmi_init(
	...
	err |= i2c_reg_write_byte_dt(
		...
		0x80
	);  // enable gyro data in FIFO, don't store header
	if (err) {
		LOG_ERR("I2C error");
	}
	return (err < 0 ? err : 0);
...
)  // this does not reset the device, to avoid clearing the config
{
	last_accel_odr = 0xff;  // reset last odr
	last_gyro_odr = 0xff;  // reset last odr
	int err
...
uint16_t bmi_fifo_read(const struct i2c_dt_spec* dev_i2c, uint8_t* data, uint16_t len) {
	...
	while (packets > 0 && len >= 6) {
		...
		uint16_t count = (uint16_t)((rawCount[1] & 1) << 8 | rawCount[0]
...
int bmi_fifo_process(uint16_t index, uint8_t* data, float g[3]) {
	...
	{
		g_bmi[i] = (int16_t)((((uint16_t)data[index + (i * 2) + 1]) << 8)
							 | data[index + (i * 2)]);
...
void bmi_accel_read(const struct i2c_dt_spec* dev_i2c, float a[3]) {
	...
	{
		a_bmi[i]
			= (int16_t)((((uint16_t)rawAccel[(i * 2) + 1]) << 8) | rawAccel[i * 2]);
...
)  // TODO: seems too sensitive? try to match icm at least // TODO: half working.
{
	uint8_t config[4] = {0};
	uint16_t* ptr = (uint16_t*)config;  // bmi is little endian
	ptr[0] = 0x7 << 13 | 0x000;  // enable all axes, set detection duration to 0
	ptr[1] = 0x1 << 15 | 0x7 << 11
		   | 0x040;  // enable any_motion, set out_conf to bit 6, set threshold (1LSB
					 // equals to 0.488mg, 64 * 0.488mg is ~31.25mg)
	int err = i2c_reg_write_byte_dt(
		dev_i2c,
		BMI270_PWR_CONF,
		0x00
