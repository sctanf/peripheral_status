/* 09/23/2017 Copyright Tlera Corporation
  ...
  the Ladybug STM32L432 Breakout Board. The MMC5983MA is a low power magnetometer, here
  used as 3 DoF in a 9 DoF absolute orientation solution.
...
static const float sensitivity
	= (1.0f / 16384.0f);  // mag sensitivity if using 18 bit data (16384 Counts/G)
...
void mmc_shutdown(const struct i2c_dt_spec* dev_i2c) {
	// reset device
	last_odr = 0xff;  // reset last odr
	int err = i2c_reg_write_byte_dt(
		dev_i2c,
		MMC5983MA_CONTROL_1,
		0x80
	);  // Don't need to wait for MMC to finish reset
	if (err) {
		LOG_ERR("I2C error");
	}
}
...
int mmc_update_odr(const struct i2c_dt_spec* dev_i2c, float time, float* actual_time) {
	...
	{  // 1000Hz*0.5ms/1000ms = 50% active
		...
		time = 1.0 / 1000;
	} else if (ODR > 100)  // Nominal working state, this should use as low power as
						   // possible
	{  // 200Hz*0.5ms/1000ms = 10% active
		MODR = MODR_200Hz;
		MBW = MBW_800Hz;  // Max MBW_200Hz, MBW_800Hz is used since the RMS noise is
						  // still only 1.2mG
		time = 1.0 / 200;
	} else if (ODR > 50) {  // 100Hz*2ms/1000ms = 20% active
		MODR = MODR_100Hz;
		MBW = MBW_400Hz;  // 0.8mG
		time = 1.0 / 100;
	} else if (ODR > 20) {  // 50Hz*4ms/1000ms = 20% active
		MODR = MODR_50Hz;
		MBW = MBW_200Hz;  // 0.6mG
		time = 1.0 / 50;
	} else if (ODR > 10) {  // 20Hz*8ms/1000ms = 16% active
		MODR = MODR_20Hz;
		MBW = MBW_100Hz;  // 0.4mG
		time = 1.0 / 20;
	} else if (ODR > 1) {  // 10Hz*8ms/1000ms = 8% active
		MODR = MODR_10Hz;
		MBW = MBW_100Hz;
		time = 1.0 / 10;
	} else if (ODR > 0) {  // 1Hz*8ms/1000ms = 0.8% active
		MODR = MODR_1Hz;
		MBW = MBW_100Hz;
		time = 1.0 / 1;
	} else {
...
void mmc_mag_read(const struct i2c_dt_spec* dev_i2c, float m[3]) {
	...
	while ((~status & 0x01) && k_uptime_get() < timeout
	) {  // wait for oneshot to complete or timeout
...
// instead, the temperature is read from the last measurement and then another
// measurement is immediately triggered
float mmc_temp_read(const struct i2c_dt_spec* dev_i2c, float bias[3]) {
	...
	// Temperature output, unsigned format. The range is -75~125°C, about 0.8°C/LSB,
	// 00000000 stands for -75°C
	float temp = rawTemp;
	temp *= 0.8f;
	temp -= 75;

	// USING SET AND RESET TO REMOVE BRIDGE OFFSET in datasheet
	if (last_rawTemp != rawTemp
		&& last_time > 1.0f / 50)  // calculate offset at low motion only
...
void mmc_mag_process(uint8_t* raw_m, float m[3]) {
	uint32_t rawMag[3];
	rawMag[0] = (uint32_t)(raw_m[0] << 10 | raw_m[1] << 2 | (raw_m[6] & 0xC0) >> 6
	);  // Turn the 18 bits into a unsigned 32-bit value
	rawMag[1] = (uint32_t)(raw_m[2] << 10 | raw_m[3] << 2 | (raw_m[6] & 0x30) >> 4
	);  // Turn the 18 bits into a unsigned 32-bit value
	rawMag[2] = (uint32_t)(raw_m[4] << 10 | raw_m[5] << 2 | (raw_m[6] & 0x0C) >> 2
	);  // Turn the 18 bits into a unsigned 32-bit value
...
const sensor_mag_t sensor_mag_mmc5983ma = {
	...
	*mmc_mag_process,
	MMC5983MA_XOUT_0  // External interface only reads 6 bytes, the data will be lower
					  // precision
};
