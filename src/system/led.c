...
static void led_pin_set(enum sys_led_color color, int brightness_pptt, int value_pptt) {
	...
	pwm_set_pulse_dt(
		&pwm_led,
		pwm_led.period / 10000 * (led_pwm_period[color][0] * value_pptt / 10000)
	);
#if PWM_LED1_EXISTS
	pwm_set_pulse_dt(
		&pwm_led1,
		pwm_led1.period / 10000 * (led_pwm_period[color][1] * value_pptt / 10000)
	);
#if PWM_LED2_EXISTS
	pwm_set_pulse_dt(
		&pwm_led2,
		pwm_led2.period / 10000 * (led_pwm_period[color][2] * value_pptt / 10000)
	);
#endif
#endif
#else
...
void set_led(enum sys_led_pattern led_pattern, int priority) {
	...
	} else if (k_current_get() != led_thread_id)  // do not suspend if called from
												  // thread
	{
		k_thread_suspend(led_thread_id);
	...
	while (1) {
		...
		switch (current_led_pattern) {
			...
			case SYS_LED_PATTERN_ACTIVE_PERSIST:  // off duration first because the
												  // device may turn on multiple times
												  // rapidly and waste battery power
				led_pattern_state = (led_pattern_state + 1) % 2;
				led_pin_set(SYS_LED_COLOR_DEFAULT, 10000, !led_pattern_state * 10000);
